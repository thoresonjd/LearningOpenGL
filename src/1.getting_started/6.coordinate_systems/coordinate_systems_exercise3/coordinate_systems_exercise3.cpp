 ...
 * @brief Every third cube rotates over time
...
namespace {
	...
	const char* WINDOW_NAME = "Coordinate Systems: Exercise 3";
	...
	const char* VERTEX_SHADER_PATH = "src/1.getting_started/6.coordinate_systems/coordinate_systems_exercise3/shader.vs";
	const char* FRAGMENT_SHADER_PATH = "src/1.getting_started/6.coordinate_systems/coordinate_systems_exercise3/shader.fs";
}
...
int main(void) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glBindVertexArray(vao);
		for (int i = 0; i < 10; i++) {
			glm::mat4 model = glm::translate(IDENTITY_MATRIX, cubePositions[i]);
			float angle = 20.0f * i;
			if (i % 3 == 0)
				angle += 30.0f * static_cast<float>(glfwGetTime());
