/**
 * @file cubemaps_skybox.cpp
 * @brief Rendering a skybox via a cubemap
 * @date July 2023
...
const char* WINDOW_NAME = "Cubemaps: skybox";
// shaders
const char* OBJECT_VERT_SHADER = "src/4.advanced_opengl/6.cubemaps/cubemaps_skybox/cubemaps.vs";
const char* OBJECT_FRAG_SHADER = "src/4.advanced_opengl/6.cubemaps/cubemaps_skybox/cubemaps.fs";
const char* SKYBOX_VERT_SHADER = "src/4.advanced_opengl/6.cubemaps/cubemaps_skybox/skybox.vs";
const char* SKYBOX_FRAG_SHADER = "src/4.advanced_opengl/6.cubemaps/cubemaps_skybox/skybox.fs";
// textures
...
unsigned int loadCubemap(std::vector<std::string> faces) {
	...
	for (unsigned int i = 0; i < faces.size(); i++) {
		unsigned char* data = stbi_load((CUBEMAP_DIR + faces[i]).c_str(), &width, &height, &nrChannels, 0);
		if (data)
