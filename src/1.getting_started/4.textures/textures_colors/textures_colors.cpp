/**
 * @file textures_colors.cpp
 * @brief Rendering a container texture mixed with vertex colors
...
namespace {
	...
	const char* WINDOW_NAME = "Textures: Colors";
	const char* TEX_PATH = "assets/textures/container.jpg";
	const char* VERTEX_SHADER_PATH = "src/1.getting_started/4.textures/textures_colors/shader.vs";
	const char* FRAGMENT_SHADER_PATH = "src/1.getting_started/4.textures/textures_colors/shader.fs";
}
...
int main(void) {
	...
	// set texture wrapping/filtering options to the current bound texture object
	...
	unsigned char* data = stbi_load(TEX_PATH, &width, &height, &numChannels, 0);
	if (data) {
