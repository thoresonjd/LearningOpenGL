/**
 * @file textures_exercise4.cpp
 * @brief Rendering two combined textures and adjusting the mix level using keys on the keyboard
...
namespace {
	...
	const char* WINDOW_NAME = "Textures: Exercise 4";
	...
	const char* VERTEX_SHADER_PATH = "src/1.getting_started/4.textures/textures_exercise4/shader.vs";
	const char* FRAGMENT_SHADER_PATH = "src/1.getting_started/4.textures/textures_exercise4/shader.fs";
	constexpr float MIX_LEVEL_MIN = 0.0f;
	constexpr float MIX_LEVEL_MAX = 1.0f;
	constexpr float MIX_LEVEL_DELTA = 0.0005f;
	float mixLevel = 0.5f;
}
...
int main(void) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// draw textures
		shader.use();
		shader.setFloat("mixIntensity", mixLevel);
...
void processInput(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS && mixLevel < MIX_LEVEL_MAX)
		mixLevel += MIX_LEVEL_DELTA;
	if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS && mixLevel > MIX_LEVEL_MIN)
		mixLevel -= MIX_LEVEL_DELTA;
}